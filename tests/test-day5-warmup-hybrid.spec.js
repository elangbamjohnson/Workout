...
test.describe('Day 5 Hybrid Warm-up Session Verification', () => {
    ...
    test('Day 5 Warm-up renders with sequential 1..6 badges, video demos, cues, right checkboxes, and continuous timer', async ({ page }) => {
        // Navigate to Day 5
        ...
        // Verify duration stat in header
        await expect(wuCard.locator('.stat-item')).toContainText('~5:30');
        ...
        const expectedDrills = [
            { name: 'Jump Rope', stat: '2 min', cue: 'Start relaxed. Light feet, relaxed shoulders', videoId: 'Gt9hlRMXDXc' },
            { name: 'Jumping Jacks', stat: '30s', cue: 'Full arm extension overhead', videoId: 'bT2iY8IjEU0' },
            { name: 'Mountain Climbers', stat: '30s', cue: 'Keep hips level', videoId: '0LvR42Z599c' },
            { name: 'Shoulder Circles', stat: '30s', cue: 'Controlled full range of motion', videoId: 'UGg2V8npqeM' },
            { name: 'Hip Circles', stat: '30s', cue: 'Wide controlled circles', videoId: 'PZFKu9583Ms' },
            { name: 'Shadowboxing', stat: '1 min 30s', cue: '1-2, hooks, body shots, slips, pivots', videoId: null }
        ];
        ...
        await rows.nth(1).locator('.btn-check').click(); // Clicking Jumping Jacks starts timer or marks it
        // Since Jumping Jacks is timed, it starts timer modal
        ...
        await page.evaluate(() => {
            Store.logItem(5, 'day5-wu1', { completed: true });
            reRenderViewingDay();
        });
        ...
        await page.screenshot({ path: path.join(outDir, 'day5-warmup-hybrid-desktop.png'), fullPage: true });

        await page.setViewportSize({ width: 375, height: 2600 });
        await page.screenshot({ path: path.join(outDir, 'day5-warmup-hybrid-mobile.png'), fullPage: true });
    });
});
